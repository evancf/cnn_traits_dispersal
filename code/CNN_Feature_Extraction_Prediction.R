...
# make predictions
target_pred = predict(model, pred_dataset)

# save predictions
target_pred_df <- as.data.frame(target_pred)
target_pred_df <- target_pred_df$V1
# round predictions
...
data_full <- cbind(as.character(pred_img), joined_img['species'], joined_img['pic_name'], pred_ref, target_pred_df, target_pred_round)
